...
bool tcpserver_acl_dbus_init(struct tcpserver_acl *acl)
{
	if (!l_dbus_register_interface(dbus_get_bus(), TCPSERVER_ACL_IFACE,
	...
	if (!l_dbus_object_add_interface(dbus_get_bus(), acl->dbus_path,
