...
static bool buffer_write(struct l_io *l_io, void *user_data)
{
	...
	(void)l_ringbuf_write(io->l_out_ringbuf, l_io_get_fd(l_io));

	return l_ringbuf_len(io->l_out_ringbuf);
}
...
static bool simple_write(struct silvair_io *io,
				...
				size_t size)
{
	if (l_ringbuf_avail(io->l_out_ringbuf) < size)
		l_warn("Write failed. Not enough space in the buffer.");
	else
		(void)l_ringbuf_append(io->l_out_ringbuf, buf, size);
				...
				void *context)
{
	...
	io->l_out_ringbuf = l_ringbuf_new(OUT_RINGBUF_SIZE);
...
void silvair_io_destroy(struct silvair_io *io)
{
	...
	if (io->l_out_ringbuf)
        l_ringbuf_free(io->l_out_ringbuf);
