				...
				void *user_data)
{
	...
	while (adv < (const uint8_t *)rx_pld + pkt_hdr->pld_len) {
		...
		cb->process_packet_cb(io, rssi,
				instant, adv + 1, field_len, user_data);
				...
				void *user_data)
{
	...
	case SILVAIR_EVT_RX:
		process_evt_rx(io, instant, pkt_hdr, len, cb, user_data);
		break;
				...
				send_data_cb cb)
{
	...
	if (!cb(io, instant, &end, 1))
		return false;

	for (uint8_t *i = buf; i != buf + size; ++i) {
		switch (*i) {
		case SLIP_END:
			if (!cb(io, instant, esc_end, 2))
				return false;
			break;

		case SLIP_ESC:
			if (!cb(io, instant, esc_esc, 2))
				return false;
			break;

		default:
			if (cb(io, instant, i, 1) != 1)
				return false;
		}
	}
...
static bool send_slip(struct silvair_io *io,
				uint8_t *buf,
				size_t size,
				uint32_t instant,
				send_data_cb cb,
			...
			void *user_data)
{
	if (io->slip.kernel_support)
		...
		process_slip(io, &io->slip, buf, size, instant, cb, user_data);
}
...
{
	if (io->slip.kernel_support) {
		if (!send_packet(io, buf, size, instant,
						cb, PACKET_TYPE_MESSAGE)) {
			l_error("write failed: %s", strerror(errno));
			return;
		}
	}
	else {
		if (!send_slip(io, buf, size, instant,
						cb, PACKET_TYPE_MESSAGE)) {
			l_error("write failed: %s", strerror(errno));
			return;
		}
