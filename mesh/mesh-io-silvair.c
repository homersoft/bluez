...
static bool silvair_io_init(struct mesh_io *io, void *opts)
{
	...
	io->pvt->tx_timeout = l_timeout_create_ms(0, send_timeout, io->pvt, NULL);
	return true;
}
...
static void send_flush(struct mesh_io_private *pvt)
{
	...
	do {
		...
		pkt_hdr->pld_len = sizeof(*tx_hdr) + sizeof(*tx_pld) + tx->len + 1;
		pkt_hdr->version = 1;
		...
		tx_pld->header.type = 2; // ADV_NOCONN_IND
		tx_pld->header.size = 6 + tx->len + 1; // bdaddress + type tag + data
...
static bool silvair_io_cancel(struct mesh_io *io, const uint8_t *data, uint8_t len)
{
	struct mesh_io_private *pvt = io->pvt;
	struct tx_pkt *tx;
	...
	if (tx)
		l_timeout_modify_ms(pvt->tx_timeout, tx->instant - get_instant());
